...
`docs/solutions/` does not exist, so there are no captured institutional learnings. The two in-repo post-mortems (`docs/SYNC_MISMATCH_BUG_ANALYSIS.md`, `docs/MISMATCH_FILTERING_ANALYSIS.md`) serve that role. Their analysis is sound and their DATA1 worked example is the best acceptance case available — but their "Files Modified" sections describe fixes that were never wired into the running code, and both contain live UUIDs, real hostnames and a full `ssh spoke1-san` command.
...
- **Normalise `pool` / `dataset` / `snapshot` once, at the ingest boundary.** Production data contains `hubpool1/hubpool1/DATA3` — the dataset arriving already pool-prefixed while `pool` is also set. Seven `if "/" in dataset:` heuristics scattered through the command generator exist to paper over this. Fixing it at ingest deletes the whole class.
...
- Replace internal identifiers (`spoke1`, `hub1`, `hqsNpN` pools, `L1S…`/`M1S…` dataset names, `ssh spoke1-san`) with neutral fixtures. Keep the *shape* of the scenarios — they are the regression cases that matter.
...
**Approach:**
- Normalise `pool`, `dataset` and snapshot name once, here. Production data contains `dataset = "hubpool1/DATA3"` with `pool = "hubpool1"` already set, producing stored paths like `hubpool1/hubpool1/DATA3`. Establish one canonical form and enforce it at the boundary; this is what allows the seven `if "/" in dataset:` heuristics to be deleted in U11.
...
- Mark the three incident post-mortems as historical, sanitised of live UUIDs, hostnames and the `ssh spoke1-san` command. Their analysis is worth keeping; their "Files Modified" sections describe fixes that were never wired in and must say so.
