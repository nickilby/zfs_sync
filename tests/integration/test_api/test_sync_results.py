...
class TestReportingAnOutcome:
    def test_a_success_flips_the_pair_to_in_sync(self, test_client, test_db):
        group, hub, (spoke,) = build_fleet(test_db)

        instructions = test_client.get(f"/api/v1/sync/instructions/{hub.id}").json()
        ...
        summary = test_client.get(f"/api/v1/sync/groups/{group.id}/status").json()
        assert summary["in_sync_count"] == 1
        assert summary["total_states"] == 1

    def test_a_failure_is_visible_with_its_message(self, test_client, test_db):
        group, hub, (spoke,) = build_fleet(test_db)
        ...
        states = test_client.get(f"/api/v1/sync/groups/{group.id}/states").json()
        assert "destination modified" in states[0]["error_message"]

    def test_history_records_every_attempt(self, test_client, test_db):
        group, hub, (spoke,) = build_fleet(test_db)
        ...
        runs = test_client.get(f"/api/v1/sync/groups/{group.id}/runs").json()
    ...
    def test_each_target_is_tracked_independently(self, test_client, test_db):
...
class TestRejectedReports:
    def test_an_unknown_status_is_rejected(self, test_client, test_db):
        group, hub, (spoke,) = build_fleet(test_db)
    ...
    def test_a_negative_byte_count_is_rejected(self, test_client, test_db):
        group, hub, (spoke,) = build_fleet(test_db)
...
class TestTheLoopIsClosed:
    def test_instruction_then_report_then_no_further_instruction(
        ...
        before = test_client.get(f"/api/v1/sync/instructions/{hub.id}").json()
        ...
        after = test_client.get(f"/api/v1/sync/instructions/{hub.id}").json()
