...
class TestUnknownSyncGroup:
    ...
    def test_decisions_returns_404(self, test_client):
        response = test_client.get(f"/api/v1/sync/groups/{self.UNKNOWN}/decisions")

        assert response.status_code == status.HTTP_404_NOT_FOUND
        assert response.json()["error"]["code"] == "sync_group_not_found"

    def test_analysis_returns_404(self, test_client):
        response = test_client.get(f"/api/v1/sync/groups/{self.UNKNOWN}/analysis")

        assert response.status_code == status.HTTP_404_NOT_FOUND
        assert response.json()["error"]["code"] == "sync_group_not_found"

    def test_both_routes_agree(self, test_client):
        decisions = test_client.get(f"/api/v1/sync/groups/{self.UNKNOWN}/decisions")
        analysis = test_client.get(f"/api/v1/sync/groups/{self.UNKNOWN}/analysis")

        assert decisions.status_code == analysis.status_code
        assert decisions.json()["error"]["code"] == analysis.json()["error"]["code"]

    def test_the_envelope_names_the_group(self, test_client):
        response = test_client.get(f"/api/v1/sync/groups/{self.UNKNOWN}/decisions")

        assert response.json()["error"]["sync_group_id"] == self.UNKNOWN

    def test_detail_is_retained_for_existing_clients(self, test_client):
        response = test_client.get(f"/api/v1/sync/groups/{self.UNKNOWN}/decisions")
...
class TestValidationErrors:
    def test_a_malformed_uuid_is_a_422_with_the_envelope(self, test_client):
        response = test_client.get("/api/v1/sync/groups/not-a-uuid/decisions")
...
class TestUnhandledErrors:
    def test_a_correlation_id_is_returned_and_logged(self, test_db, monkeypatch, caplog):
        ...
        # TestClient re-raises instead of letting it through.
