...
def upgrade() -> None:
    ...
    op.add_column('sync_groups', sa.Column('directional', sa.Boolean(), nullable=False, server_default='false'))
    
    # Add hub_system_id column (nullable, only used when directional=True)
    op.add_column('sync_groups', sa.Column('hub_system_id', postgresql.UUID(as_uuid=True), nullable=True))
